...
$(document).ready(function(){
	function getCourse(all_balance){
		$.ajax({
	        url: "https://vbote.xyz/currency.php",
	        success: function(response) {
	        	$("#bot_convert").text("$" + (1 / Number(response.ath_usd) * all_balance).toFixed(4));
	        },
	        dataType: "json",
	    });
	}
    ...
    $.ajax({
        type: "POST",
        url: "https://vbote.xyz/ajax/",
        success: function(response) {
        	var i = 0;
        	var all_balance = 0;
        	for(var bot in response.bots){
				switch (response.bots[i].status) {
				  case "starting":
	            ...
	            $("#block_bots").append('<tr><td><div class="d-flex no-block align-items-center"><div class="mr-2"><img src="/assets/images/users/d2.jpg" alt="user" class="rounded-circle" width="45" /></div><div class=""><h5 class="mb-0 font-16 font-medium">'+response.bots[i].wax_name+'</h5><span></span></div></div></td><td>Elite Admin</td><td><i class="fa fa-circle '+status+'" data-toggle="tooltip" data-placement="top" title="'+response.bots[i].status+'"></i></td><td>'+response.bots[i].next_pay+'</td><td class="blue-grey-text  text-darken-4 font-medium">'+response.bots[i].balance+' TLM</td></tr>');
        		all_balance = Number(all_balance) + Number(response.bots[i].balance);
        		i++;
        	}
        	$("#bot_balance").text("TLM " + all_balance);
        	$("#bot_running").text(i)
        	getCourse(all_balance);
